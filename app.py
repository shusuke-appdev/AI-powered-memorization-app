...
from auth import (
    ...
    delete_session,
    get_api_key,
    get_daily_quota_limit,
    get_username,
    register_user,
    update_api_key,
    update_daily_quota_limit,
...
from gemini_client import help_chat
    ...
    with col2:
        ...
        with register_tab:
            with st.form("register_form"):
                ...
                if st.form_submit_button(
                    "登録", type="primary", use_container_width=True
                ):
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        # ヘルプAI チャットセクション
        st.markdown(
            "<div class='help-ai-title'>🤖 ヘルプAI</div>", unsafe_allow_html=True
        )
                ...
                if not api_key:
                    ...
                    with st.spinner("回答中..."):
                        result = help_chat(
                            user_question,
                            api_key,
                            st.session_state.help_chat_history[:-1],
                        )
                    if result["success"]:
                        st.session_state.help_chat_history.append(
...
""",
                    ...
                    related_cards_for_fav = [
                        c for c in cards if c.get("source_id") == current_source["id"]
                    ]
                    is_source_fav = any(
                        c.get("is_favorite", False) for c in related_cards_for_fav
                    )
                    ...
                    with nav_col2:
                        if st.button(
                            "✓ 復習を終了", type="primary", use_container_width=True
                        ):
                            st.session_state.reviewed_source_ids = []
                            st.session_state.source_review_index = 0
                            st.rerun()
    ...
    with tab2:
        ...
        # BLANK_ENABLED_TYPES = ["規範", "判例"]  # 穴埋めあり (Unused)
        ...
        selected_category = (
            selected_category_raw
            if selected_category_raw != "-- カテゴリを選択 --"
            else ""
        )
        ...
        selected_type = (
            selected_type_raw if selected_type_raw != "-- タイプを選択 --" else ""
        )
        ...
        if is_blank_disabled:
            ...
            if manual_mode:
                st.info(
                    "💡 穴埋めにしたい箇所を【】で囲んでください。例: 民法【709条】は..."
                )
            ...
            if manual_mode and "generated_cards" in st.session_state:
                ...
                if source_text != prev_text:
                    ...
                    st.info(
                        "テキストが変更されました。再度「カード生成」を押してください。"
                    )
            if manual_mode:
            ...
            if manual_mode:
                # 手動モード: 【】マーカーで直接カード生成
                if st.button(
                    "✨ カード生成", type="primary", key="manual_generate_btn"
                ):
                    if not source_text:
                        st.warning("テキストを入力してください。")
                    elif "【" not in source_text or "】" not in source_text:
                        st.warning(
                            "【】で穴埋め箇所を指定してください。例: 民法【709条】は..."
                        )
                    else:
                ...
                if st.button("📝 テキストを解析", type="primary"):
                    if not source_text:
                        ...
                        with st.spinner("AIがテキストを解析中..."):
                            ...
                            # エラーチェック
                            if (
                                isinstance(phrases, dict)
                                and phrases.get("error") == "API_QUOTA_EXCEEDED"
                            ):
                                st.error(
                                    f"⚠️ {phrases.get('message', 'APIの利用制限に達しました。')}"
                                )
                            elif phrases:
                                st.session_state.phrases = phrases
                                st.session_state.selected_indices = []
                                st.success(
                                    f"{len(phrases)}個の文節に分割しました。穴埋め箇所を選択してください。"
                                )
                            else:
        ...
        if "phrases" in st.session_state and st.session_state.phrases:
                    ...
                    if api_key:
                        with st.spinner("AIが提案中..."):
                            ...
                            if (
                                ...
                                and suggested.get("error") == "API_QUOTA_EXCEEDED"
                            ):
                                st.error(
                                    f"⚠️ {suggested.get('message', 'APIの利用制限に達しました。')}"
                                )
                            else:
            ...
            for i, phrase in enumerate(phrases):
                ...
                if is_punctuation:
                    phrase_buttons_html.append(
                        f"<span class='phrase-toggle punct'>{phrase}</span>"
                    )
                elif is_selected:
                    phrase_buttons_html.append(
                        f"<span class='phrase-toggle selected' data-idx='{i}'>{phrase}</span>"
                    )
                else:
                    phrase_buttons_html.append(
                        f"<span class='phrase-toggle normal' data-idx='{i}'>{phrase}</span>"
                    )

            # HTMLでプレビュー表示
            st.markdown(
                f"<div class='phrase-grid'>{''.join(phrase_buttons_html)}</div>",
                unsafe_allow_html=True,
            )
            ...
            if selectable_phrases:
                # 行ごとに分割
                rows = [
                    selectable_phrases[i : i + 4]
                    for i in range(0, len(selectable_phrases), 4)
                ]
                ...
                for row in rows:
                    ...
                    for col_idx, (phrase_idx, phrase_text) in enumerate(row):
                        with cols[col_idx]:
                            is_selected = (
                                phrase_idx in st.session_state.selected_indices
                            )
                            btn_label = (
                                f"✓ {phrase_text}" if is_selected else phrase_text
                            )
                            btn_type = "primary" if is_selected else "secondary"
                            if st.button(
                                btn_label,
                                key=f"toggle_{phrase_idx}",
                                type=btn_type,
                                use_container_width=True,
                            ):
            ...
            if selected:
                ...
                st.markdown(
                    f"**穴埋め箇所: {len(answer_groups)}個** (隣接ブロックは自動結合)"
                )
            ...
            with st.form("save_cards_form"):
                ...
                with submit_col1:
                    if st.form_submit_button("💾 デッキに保存", type="primary"):
                        ...
                        original_text = (
                            st.session_state.add_card_text
                            if "add_card_text" in st.session_state
                            else ""
                        )
                        source_id = None
                        if original_text:
                        ...
                        st.success(
                            f"{count} 枚のカードを保存しました！（原文カードも保存済み）"
                        )
                        # 全ての工程をクリア
    ...
    with tab5:
        ...
        # 教科選択
        selected_category_audio = st.selectbox(
            "教科を選択", CATEGORIES, key="audio_category"
        )
    ...
    with tab3:
        ...
        if not source_cards and not cards:
            ...
            # 統計表示
            st.markdown(
                f"**原文カード: {len(source_cards)} 件 / 暗記カード: {len(cards)} 枚**"
            )
            ...
            for i, category in enumerate(CATEGORIES):
                with tabs[i]:
                    ...
                    category_sources = [
                        ...
                        if s.get("category", "その他") == category
                    ]
                    ...
                    if selected_type_filter != "すべて":
                        category_sources = [
                            ...
                            if s.get("card_type") == selected_type_filter
                        ]
                    ...
                    if not category_sources and not orphan_cards:
                        ...
                        for sc in category_sources:
                            ...
                            # この原文に紐づく暗記カード
                            linked_cards = [
                                c for c in cards if c.get("source_id") == source_id
                            ]

                            # Expander: 原文カード（紐づきカード数も表示）
                            with st.expander(
                                f"📄 {source_title}（暗記カード {len(linked_cards)} 枚）",
                                expanded=False,
                            ):
                                # 原文表示・編集
                                st.markdown("**📝 原文**")
                                ...
                                # お気に入り状態判定
                                is_source_fav = any(
                                    c.get("is_favorite", False) for c in linked_cards
                                )
                                ...
                                with btn_col2:
                                    ...
                                    if st.button(
                                        fav_label,
                                        key=f"edit_fav_{source_id}",
                                        use_container_width=True,
                                    ):
                                        toggle_favorite_by_source_id(
                                            user_id, source_id, not is_source_fav
                                        )
                                        st.rerun()
                                ...
                                with btn_col3:
                                    if st.button(
                                        "🗑️ 全削除",
                                        key=f"del_all_{source_id}",
                                        use_container_width=True,
                                    ):
                                        st.session_state[
                                            f"confirm_del_all_{source_id}"
                                        ] = True

                                if st.session_state.get(
                                    f"confirm_del_all_{source_id}", False
                                ):
                                    st.warning(
                                        "⚠️ この原文カードと紐づく暗記カードを全て削除しますか？"
                                    )
                                    c1, c2, c3 = st.columns([1, 1, 3])
                                    with c1:
                                        if st.button(
                                            "✓ 削除",
                                            key=f"yes_del_all_{source_id}",
                                            type="primary",
                                        ):
                                            # 暗記カード削除
                                            for card in linked_cards:
                                    ...
                                    with c2:
                                        if st.button(
                                            "✗ 戻る", key=f"no_del_all_{source_id}"
                                        ):
                                            del st.session_state[
                                                f"confirm_del_all_{source_id}"
                                            ]
                        ...
                        if orphan_cards:
                            ...
                            for card in orphan_cards:
                                with st.expander(
                                    f"🎴 {card.get('title', '無題')}: {card['question'][:30]}..."
                                ):
                                    with st.form(key=f"orphan_form_{card['id']}"):
                                        new_q = st.text_input(
                                            "問題", value=card["question"]
                                        )
                                        new_a = st.text_input(
                                            "答え", value=card["answer"]
                                        )
                                    ...
                                    if st.button(
                                        "🗑️ 削除", key=f"del_orphan_{card['id']}"
                                    ):
                                        delete_card(user_id, card["id"])
                                        st.success("削除しました")
                                        st.rerun()
